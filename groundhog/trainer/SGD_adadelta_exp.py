...
class SGD(object):
    def __init__(self,
        ...
        self.burn_in_steps = self.state.get('burn_in_steps',numpy.ceil(1.0/(1.0-self.state['adarho'])))
        
        self.param_shrinker = theano.shared( numpy.int32(0), name='param_shrinker' )
        ...
        if 'cutoff' in state and state['cutoff'] > 0:
            ...
            notfinite = TT.or_(TT.or_(TT.isnan(norm_gs), TT.isinf(norm_gs)), self.param_shrinker)
            _gs = []
        ...
        if state['cutoff_adapt']:
            ...
            cut_rho_mean2= cut_rho_mean #numpy.float32(self.state['cutoff_rho'])
    ...
    def __call__(self):
        ...
        if self.state.get('shrink_period'):
            if self.step % self.state.get('shrink_period') == 0:
                print "shrinking params!!"
                self.param_shrinker.set_value(1)
            else:
                self.param_shrinker.set_value(0)
